...
final class PlayerSettings: ObservableObject {
    ...
    @Published var autoNextEpisode: Bool { didSet { save("player.autoNextEpisode", autoNextEpisode) } }
    ...
    private init() {
        ...
        autoNextEpisode = defaults.object(forKey: "player.autoNextEpisode") as? Bool ?? true
    ...
    var body: some View {
        Form {
            Section("播放") {
                ...
                Toggle("自動播放下一集", isOn: $settings.autoNextEpisode)
            }
