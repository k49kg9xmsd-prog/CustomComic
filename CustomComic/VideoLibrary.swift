...
final class VideoLibrary: ObservableObject {
    ...
    func exportURL(for item: VideoItem) -> URL { videoURL(for: item) }

    func orderedEpisodes(around item: VideoItem) -> [VideoItem] {
        let series = item.title.components(separatedBy: " · ").first ?? item.title
        let matches = videos.filter { candidate in
            candidate.category == item.category && ((candidate.title.components(separatedBy: " · ").first ?? candidate.title) == series)
        }
