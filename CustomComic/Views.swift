...
struct LibraryView: View {
    ...
    private let columns = [GridItem(.adaptive(minimum: 145), spacing: 18)]
            ...
            Form {
                Section("作品資料") {
                    ...
                    Button {
                        ...
                        HStack {
                            Label("自訂作品封面", systemImage: "photo")
                            Spacer()
...
struct ReaderEntryView: View {
    ...
    var body: some View {
        Group {
            if let episode, let destinationPage {
                ReaderView(
                    bookID: bookID,
                    episodeID: episodeID,
            ...
            Group {
                if auth.isUnlocked {
                    if library.hiddenBooks.isEmpty {
                        ...
                        ScrollView {
                            LazyVGrid(columns: columns, spacing: 22) {
                                ForEach(library.hiddenBooks) { book in
                                    NavigationLink {
                                        SeriesDetailView(bookID: book.id)
                                    } label: {
                                        BookCard(book: book)
