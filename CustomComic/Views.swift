...
struct RootView: View {
    ...
    @AppStorage("contentMode") private var contentModeRaw = ContentMode.comic.rawValue

    private var contentMode: ContentMode {
        get { ContentMode(rawValue: contentModeRaw) ?? .comic }
            ...
            ZStack(alignment: .bottomTrailing) {
                ...
                Button {
                    withAnimation(.spring(response: 0.35, dampingFraction: 0.72)) {
                        contentMode = contentMode == .comic ? .animation : .comic
                    }
                } label: {
                    Label(
                        contentMode == .comic ? "切換動畫" : "切換漫畫",
                        systemImage: contentMode == .comic ? "play.rectangle.fill" : "books.vertical.fill"
                    )
                    .font(.headline)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .principal) {
                    Picker("內容", selection: $selectedTab) {
                        Text(contentMode == .comic ? "本地漫畫" : "本地動畫").tag(LibraryTab.local)
                        Text(contentMode == .comic ? "網頁" : "線上模式").tag(LibraryTab.web)
                    }
