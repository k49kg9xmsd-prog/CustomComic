...
private struct ThinBlackOutline: ViewModifier {
    func body(content: Content) -> some View {
        content
            .shadow(color: .black.opacity(0.9), radius: 0, x: 0.7, y: 0)
            .shadow(color: .black.opacity(0.9), radius: 0, x: -0.7, y: 0)
            .shadow(color: .black.opacity(0.9), radius: 0, x: 0, y: 0.7)
            .shadow(color: .black.opacity(0.9), radius: 0, x: 0, y: -0.7)
    }
}

private extension View {
    func thinBlackOutline() -> some View {
        modifier(ThinBlackOutline())
    }
}
...
struct RootView: View {
    ...
    @AppStorage("libraryMode") private var libraryModeRaw = LibraryMode.comic.rawValue

    private var libraryMode: LibraryMode {
        get { LibraryMode(rawValue: libraryModeRaw) ?? .comic }
        nonmutating set { libraryModeRaw = newValue.rawValue }
    }
    ...
    var body: some View {
        NavigationStack {
            ZStack(alignment: .bottomTrailing) {
                ...
                Group {
                    if libraryMode == .comic {
                        if selectedTab == .local { LibraryView() }
                        else { WebsiteListView() }
                    } else {
                        if selectedTab == .local { VideoLibraryView() }
                        else { OnlineVideoImportView() }
                    }
                }
                ...
                Button {
                    withAnimation(.spring(response: 0.35, dampingFraction: 0.72)) {
                        libraryMode = libraryMode == .comic ? .animation : .comic
                    }
                } label: {
                    Label(
                        libraryMode == .comic ? "切換動畫" : "切換漫畫",
                        systemImage: libraryMode == .comic ? "play.rectangle.fill" : "books.vertical.fill"
                    )
                    .font(.headline)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .principal) {
                    Picker("內容", selection: $selectedTab) {
                        Text(libraryMode == .comic ? "本地漫畫" : "本地動畫").tag(LibraryTab.local)
                        Text(libraryMode == .comic ? "網頁" : "線上模式").tag(LibraryTab.web)
                    }
...
struct LibraryView: View {
    ...
    var body: some View {
            ...
            .sheet(isPresented: $showingBatchManager) {
                ComicBatchManagerView()
            }
            .fullScreenCover(isPresented: $showingHiddenLibrary) {
                HiddenLibraryView()
            }
...
struct ComicBatchManagerView: View {
    ...
    var body: some View {
        NavigationStack {
            List {
                        ...
                        Button {
                            ...
                            HStack {
                                ...
                                VStack(alignment: .leading) {
                                    Text(book.title)
                                    Text("\(book.category) · \(book.episodes.count) 集").font(.caption).foregroundStyle(.secondary)
                                }
                            }
                        }
                        .buttonStyle(.plain)
                ...
                if !selected.isEmpty {
                    Section("批次操作") {
                        ...
                        Button("套用分類") { library.batchSetCategory(ids: selected, category: category); selected.removeAll() }
                        Button("移到隱藏庫") { library.batchSetHidden(ids: selected, hidden: true); selected.removeAll() }
            ...
            .toolbar {
                ToolbarItem(placement: .topBarLeading) { Button("全選") { selected = Set(library.visibleBooks.map(\.id)) } }
                ToolbarItem(placement: .topBarTrailing) { Button("完成") { dismiss() } }
            }
            .confirmationDialog("確定刪除已選作品？", isPresented: $showingDelete) {
                Button("刪除", role: .destructive) { library.batchDelete(ids: selected); selected.removeAll() }
            }
        }
    }
}
...
struct BookNavigationCard: View {
    ...
    var body: some View {
        ...
        .contextMenu {
            ...
            Button {
                do { exportURL = try library.exportZIP(for: book) }
                catch { exportError = error.localizedDescription }
            } label: {
                Label("匯出 ZIP", systemImage: "square.and.arrow.up")
            }
        ...
        .sheet(item: Binding(
            get: { exportURL.map { ComicExportURL(value: $0) } },
            set: { exportURL = $0?.value }
        )) { value in
...
private struct ComicExportURL: Identifiable {
    let id = UUID()
    let value: URL
}
        ...
        ZStack {
            ...
            if !urls.isEmpty {
                if mode == .vertical {
                    ...
                    ZoomableImagePage(url: urls[page]) { normalizedX in
                        if normalizedX < 0.32 {
                            if page > 0 { page -= 1 }
            ...
            if isScrubbing, urls.indices.contains(previewPage) {
                VStack(spacing: 6) {
                    LocalImage(url: urls[previewPage], contentMode: .fit, maxPixelSize: 420)
        ...
        .toolbar {
            ...
            ToolbarItem(placement: .bottomBar) {
                VStack(spacing: 2) {
                    Slider(
                        value: Binding(
                            get: { Double(isScrubbing ? previewPage : page) },
                            set: { newValue in
                                // 拖曳時只更新輕量縮圖；放開後才真正跳頁，避免每格都重排閱讀器。
                                previewPage = min(
                    ...
                    Text("\((isScrubbing ? previewPage : page) + 1) / \(urls.count)")
                        .font(.caption.monospacedDigit())
                }
            }
        }
        .onAppear {
            appearance.beginReading()
        }
        .onDisappear {
            appearance.endReading()
        }
...
struct HiddenLibraryView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .searchable(text: $searchText, prompt: "搜尋隱藏作品、分類或集數")
            .toolbar {
                ToolbarItem(placement: .cancellationAction) {
                    Button("關閉") { dismiss() }
                }
