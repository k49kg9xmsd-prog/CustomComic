...
struct VideoPlayerScreen: View {
        ...
        Group {
            if let item {
                VideoPlayer(player: player)
                    ...
                    .onAppear { prepare(item) }
                    .onDisappear { saveProgress(item); cleanup() }
            } else {
                VStack(spacing: 12) { Image(systemName: "exclamationmark.triangle").font(.largeTitle); Text("找不到影片").font(.headline) }
            }
        }
    }
    ...
    private func prepare(_ item: VideoItem) {
        ...
        hasPrepared = true
        player.replaceCurrentItem(with: AVPlayerItem(url: videoLibrary.videoURL(for: item)))
        ...
        observer = player.addPeriodicTimeObserver(forInterval: CMTime(seconds: 2, preferredTimescale: 600), queue: .main) { _ in
            saveProgress(item)
        }
        player.play()
    }
    ...
    private func saveProgress(_ original: VideoItem) {
        ...
        let duration = player.currentItem?.duration.seconds ?? original.duration
        guard position.isFinite, duration.isFinite else { return }
        videoLibrary.updateProgress(original, position: position, duration: duration)
    }
