...
struct OnlineVideoImportView: View {
    ...
    @State private var pastedRuleJSON = ""
    ...
    var body: some View {
            ...
            if ruleStore.rules.isEmpty {
                Section("來源規則") {
                    VStack(spacing: 10) {
                        ...
                        Text("尚未匯入來源規則")
                            .font(.headline)
                        Text("匯入 JSON 規則後，App 才能依規則搜尋、取得集數並解析播放網址。")
                            .font(.footnote)
                            .foregroundStyle(.secondary)
                            .multilineTextAlignment(.center)
                    }
                    .frame(maxWidth: .infinity)
                    .padding(.vertical, 8)
                    Button("從檔案匯入規則") { showingRuleImporter = true }
                    Button("貼上 JSON 匯入") { showingPasteImporter = true }
                    Button("加入內建測試規則") { importBuiltInTestRule() }
                }
            } else {
                Section("搜尋來源") {
                    Button("已啟用 \(ruleStore.rules.count) 個來源") { showingRules = true }
                    Button("從檔案匯入更多規則") { showingRuleImporter = true }
                    Button("貼上 JSON 匯入") { showingPasteImporter = true }
                    Button("加入內建測試規則") { importBuiltInTestRule() }
                }
            }
        ...
        .sheet(isPresented: $showingRuleImporter) {
            RuleFilePicker { url in
                showingRuleImporter = false
                do {
        ...
        .sheet(isPresented: $showingPasteImporter) {
            NavigationStack {
                VStack(spacing: 12) {
                    TextEditor(text: $pastedRuleJSON)
                    ...
                    Button("匯入貼上的規則") {
                        do {
                            guard let data = pastedRuleJSON.data(using: .utf8), !data.isEmpty else {
                ...
                .padding()
                .navigationTitle("貼上規則 JSON")
                .toolbar {
                    ToolbarItem(placement: .topBarTrailing) {
                        Button("取消") { showingPasteImporter = false }
                    }
                }
            }
        }
    ...
    private func importBuiltInTestRule() {
        let json = #"""
        {
          "name": "Apple 公開 HLS 測試",
          "baseURL": "https://devstreaming-cdn.apple.com/",
          "searchURL": "https://devstreaming-cdn.apple.com/videos/streaming/examples/img_bipbop_adv_example_ts/master.m3u8",
          "resultPattern": "([\\s\\S]+)",
          "titleGroup": 1,
          "detailURLGroup": 1,
          "coverURLGroup": null,
          ...
          "episodeTitleGroup": 1,
          "episodeURLGroup": 1,
          "videoPattern": "(https://devstreaming-cdn\\.apple\\.com/videos/streaming/examples/img_bipbop_adv_example_ts/master\\.m3u8)",
          "videoURLGroup": 1
        }
        """#
        do {
            try ruleStore.importRules(from: Data(json.utf8))
        } catch {
            errorMessage = error.localizedDescription
        }
    }
