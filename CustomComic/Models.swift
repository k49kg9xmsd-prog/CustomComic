...
struct VideoItem: Identifiable, Codable, Equatable {
    ...
    var progressText: String {
        guard lastPosition > 1 else { return "尚未觀看" }
        return "\(Self.timeText(lastPosition)) / \(Self.timeText(duration))"
    }
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case createdAt, lastWatchedAt, sourceURLString, lastPosition, duration
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        lastPosition = try box.decodeIfPresent(Double.self, forKey: .lastPosition) ?? 0
        duration = try box.decodeIfPresent(Double.self, forKey: .duration) ?? 0
    }
}
